//------------------------------------------------------------------------------              //
// Definition of installed language word clocks
// Defined in ClockLogic.h
//------------------------------------------------------------------------------
extern void Fire2023_2D(void);
extern void Dutch(void);
extern void English(void);
extern void German(void); 
extern void French(void);
extern void DitLeHeure(void);
...
#define WAS     ColorLeds("was",     8,  10, SECColor);  ColorLeds("", 4, 5, Mem.DimmedLetter); Is = false;  // Turn off the IS LEDs
...
const byte MATRIX_HEIGHT =  25;                                                               // Grid size For digital display mode.
                     ...
                     #ifdef NL 
#define HET     ColorLeds("Het",      0,   2, MINColor);   
#define IS      ColorLeds("is",       4,   5, SECColor); 
#define WAS     ColorLeds("was",      8,  10, SECColor); 
...
#define X_ON    ColorLeds("",         0,   2, LetterColor);
                     #endif //NL
                     #ifdef UK
#define IT      ColorLeds("| It",   347, 348, UKMINColor);   
#define ISUK    ColorLeds("is",     344, 345, UKSECColor); 
#define WASUK   ColorLeds("was",    340, 342, UKSECColor);
...
#define OCLOCK  ColorLeds("O'clock",605, 610, UKLetterColor);
                      #endif //UK
                      #ifdef DE
#define ES      ColorLeds("\nEs",   334, 335, DEMINColor);   
#define IST     ColorLeds("ist",    330, 332, DESECColor); 
#define WAR     ColorLeds("war",    326, 328, DESECColor); 
...
#define UHR     ColorLeds("uhr",    621, 623, DELetterColor);
                      #endif //DE
                      #ifdef FR
#define IL      ColorLeds("| Il",    13,  14, FRMINColor);   
#define EST     ColorLeds("est",     16,  18, FRSECColor); 
#define ETAIT   ColorLeds("etait",   20,  24, FRSECColor);
...
const byte MATRIX_WIDTH  =  29;                                                               // Grid size For digital display mode.
const byte MATRIX_HEIGHT =  29;                                                               // Grid size For digital display mode.
...
#define MVOOR   ColorLeds("voor",    63,  66, LetterColor);
#define MTIEN   ColorLeds("tien",    52,  55, LetterColor); 
#define OVER    ColorLeds("over",   108, 111, LetterColor);
#define KWART   ColorLeds("kwart",  118, 122, LetterColor);
#define OVER2   ColorLeds("over",   156, 159, LetterColor);
#define HALF    ColorLeds("half",   164, 167, LetterColor);
#define VOOR    ColorLeds("voor",   168, 171, LetterColor);
#define EEN     ColorLeds("een",    176, 178, LetterColor);
#define DRIE    ColorLeds("drie",   212, 215, LetterColor);
#define TWEE    ColorLeds("twee",   220, 223, LetterColor);
#define VIER    ColorLeds("vier",   224, 227, LetterColor);
#define VIJF    ColorLeds("vijf",   228, 231, LetterColor);
#define ZES     ColorLeds("zes",    233, 235, LetterColor);
#define NEGEN   ColorLeds("negen",  268, 272, LetterColor);
#define ZEVEN   ColorLeds("zeven",  275, 279, LetterColor);
#define ACHT    ColorLeds("acht",   280, 283, LetterColor);
#define TIEN    ColorLeds("tien",   285, 288, LetterColor);
#define ELF     ColorLeds("elf",    289, 291, LetterColor);
#define TWAALF  ColorLeds("twaalf", 330, 335, LetterColor);
#define UUR     ColorLeds("uur",    324, 326, LetterColor);
#define MIN1    ColorLeds("M1",     672, 672, LetterColor); 
#define MIN2    ColorLeds("M2",     673, 673, LetterColor);       //MINColor); 
#define MIN3    ColorLeds("M3",     674, 674, LetterColor);
#define MIN4    ColorLeds("M4",     675, 675, LetterColor); 
#define EDSOFT { ColorLeds("EdSoft",   6,   7, LetterColor);  ColorLeds("", 46,  49, LetterColor); ColorLeds("",  56, 61, LetterColor); ColorLeds("", 101, 106, LetterColor);}
                     #endif //NL4
                     #ifdef UK4
#define IT      ColorLeds("| It",   337, 338, UKMINColor);   
#define ISUK    ColorLeds("is",     340, 341, UKSECColor); 
#define WASUK   ColorLeds("was",    340, 341, UKSECColor);
#define EXACTUK ColorLeds("exact",  386, 390, UKLetterColor);
#define HALFUK  ColorLeds("half",   381, 384, UKLetterColor); 
#define TWENTY  ColorLeds("twenty", 392, 397, UKLetterColor); 
#define MFIVE   ColorLeds("five",   400, 403, UKLetterColor);
#define QUARTER ColorLeds("quarter",441, 447, UKLetterColor);
#define MTEN    ColorLeds("ten",    436, 438, UKLetterColor);
#define PAST    ColorLeds("past",   448, 451, UKLetterColor);
#define TO      ColorLeds("to",     453, 454, UKLetterColor);
#define SIXUK   ColorLeds("six",    456, 458, UKLetterColor);
#define TWO     ColorLeds("two",    500, 502, UKLetterColor);
#define FIVE    ColorLeds("five",   493, 496, UKLetterColor);
#define TWELVE  ColorLeds("twelve", 504, 509, UKLetterColor);
#define TEN     ColorLeds("ten",    513, 515, UKLetterColor);
#define ELEVEN  ColorLeds("eleven", 554, 559, UKLetterColor);
#define FOUR    ColorLeds("four",   549, 552, UKLetterColor);
#define NINE    ColorLeds("nine",   561, 564, UKLetterColor);
#define THREE   ColorLeds("three",  567, 571, UKLetterColor);
#define EIGHT   ColorLeds("eight",  608, 612, UKLetterColor);
#define ONE     ColorLeds("one",    616, 618, UKLetterColor);
#define SEVEN   ColorLeds("seven",  622, 626, UKLetterColor);
#define OCLOCK  ColorLeds("O'clock",661, 666, UKLetterColor);
                      #endif //UK
                      #ifdef DE4
#define ES      ColorLeds("\nEs",   353, 354, DEMINColor);   
#define IST     ColorLeds("ist",    356, 358, DESECColor); 
#define WAR     ColorLeds("war",    356, 358, DESECColor); 
#define GENAU   ColorLeds("genau",  370, 374, DELetterColor);
#define MZEHN   ColorLeds("zehn",   365, 368, DELetterColor);
#define MFUNF   ColorLeds("funf",   408, 411, DELetterColor);
#define VIERTEL ColorLeds("viertel",413, 419, DELetterColor);
#define ZWANZIG ColorLeds("zwanzig",425, 431, DELetterColor);
#define KURZ    ColorLeds("kurz",   420, 423, DELetterColor);
#define VOR     ColorLeds("vor",    465, 467, DELetterColor);
#define NACH    ColorLeds("nach",   470, 473, DELetterColor);
#define HALB    ColorLeds("halb",   482, 485, DELetterColor);
#define FUNF    ColorLeds("funf",   476, 479, DELetterColor);
#define EINS    ColorLeds("eins",   520, 523, DELetterColor);
#define VIERDE  ColorLeds("vier",   525, 528, DELetterColor);
#define ZEHN    ColorLeds("zehn",   539, 542, DELetterColor);
#define ZWOLF   ColorLeds("zwolf",  532, 536, DELetterColor);
#define DREI    ColorLeds("drei",   577, 580, DELetterColor);
#define NEUN    ColorLeds("neun",   583, 586, DELetterColor);
#define ACHTDE  ColorLeds("acht",   594, 597, DELetterColor);
#define SECHS   ColorLeds("sechs",  588, 592, DELetterColor);
#define SIEBEN  ColorLeds("sieben", 633, 638, DELetterColor);
#define ZWEI    ColorLeds("zwei",   640, 643, DELetterColor);
#define ELFDE   ColorLeds("elf",    652, 654, DELetterColor);
#define UHR     ColorLeds("uhr",    645, 647, DELetterColor);
                      #endif //DE4
                      #ifdef FR4
